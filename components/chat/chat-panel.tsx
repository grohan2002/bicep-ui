...
export function ChatPanel() {
  const messages = useConversionStore((s) => s.messages);
  const streamingText = useConversionStore((s) => s.streamingText);
  ...
  if (messages.length === 0 && !isConverting) {
    return (
      <div className="flex h-full items-center justify-center text-muted-foreground text-sm">
        Run a conversion to see the agent conversation
      </div>
    );
  }
